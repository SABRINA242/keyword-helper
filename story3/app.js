...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initShintongTab() {
    ...
    var lastCommittedFolder = storedCharacterImages.folder || "";
    folderInput.value = lastCommittedFolder;

    folderInput.addEventListener("focus", function() {
       lastCommittedFolder = normalizeFolder(folderInput.value);
    });

    folderInput.addEventListener("change", function () {
      var newFolder = normalizeFolder(folderInput.value).normalize('NFC');
      if (newFolder === lastCommittedFolder) return;
      
      var oldPrefix = lastCommittedFolder ? lastCommittedFolder + "/" : "/";
      var newPrefix = newFolder ? newFolder + "/" : "/";
      
      var updateCount = 0;

      // 1. Update storedCharacterImages.paths
      if (storedCharacterImages.paths) {
        Object.keys(storedCharacterImages.paths).forEach(function(name) {
            var p = storedCharacterImages.paths[name] || "";
            p = p.normalize('NFC');
            if (p && p.startsWith(oldPrefix)) {
                 storedCharacterImages.paths[name] = newPrefix + p.slice(oldPrefix.length);
            }
        });
      }

      // 2. Update storedScenePrompts
      if (storedScenePrompts && storedScenePrompts.byLocation) {
         Object.keys(storedScenePrompts.byLocation).forEach(function(loc) {
             var text = storedScenePrompts.byLocation[loc] || "";
             if (!text) return;
             
             var lines = text.split("\n");
             var inImageSection = false;
             var newLines = lines.map(function(line) {
                 if (line.trim().includes("🖼️ [등장 인물 이미지]")) {
                     inImageSection = true;
                     return line;
                 }
                 if (line.trim().startsWith("🎬 장면") || line.trim().startsWith("📝") || line.trim().startsWith("🎯")) {
                     inImageSection = false;
                     return line;
                 }
                 
                 if (inImageSection && line.trim()) {
                     var trimmed = line.trim().normalize('NFC');
                     if (trimmed.startsWith(oldPrefix)) {
                         updateCount++;
                         return line.replace(oldPrefix, newPrefix);
                     }
                 }
                 return line;
             });
             
             storedScenePrompts.byLocation[loc] = newLines.join("\n");
         });
         saveStoredScenePrompts();
      }

      storedCharacterImages.folder = newFolder;
      lastCommittedFolder = newFolder;
      saveStoredCharacterImages();
      renderMappingList();
      
      // Update bodyOutput if visible
      if (bodyOutput && currentLocation && storedScenePrompts.byLocation[currentLocation]) {
          bodyOutput.value = storedScenePrompts.byLocation[currentLocation];
      }
      rebuildSceneCharacterControls();
      
      alert("폴더 경로가 변경되었습니다.\n\n" + 
            "이전 경로: " + oldPrefix + "\n" +
            "새 경로: " + newPrefix + "\n" +
            "업데이트된 항목 수: " + updateCount + "개");
    });
    ...
    if (buildScenePromptButton) {
      buildScenePromptButton.addEventListener("click", function () {
        ...
        var legacyKoInstruction =
          "[상황/행동/배경/조명/분위기: 위 대본내용 전체를 먼저 읽고, 인물의 외형(얼굴, 체형 등) 묘사는 하지 말고, 인물들의 행동·동선·제스처·시선과 서로 간의 상호작용, 카메라 구도와 거리감, 실내/실외 배경(공간 구조와 소품), 조명(시간대, 빛의 방향과 색감), 전체 분위기(따뜻함/긴장/불안/고요함 등)를 배우와 연출팀이 그대로 촬영할 수 있을 정도로 구체적인 한국어 문장으로 묘사해 주세요. ★절대 원칙: 대본에 없는 내용을 절대로 추가하지 마세요. 특히, 인물이 하지 않은 행동(예: 물건을 사거나, 음식을 먹거나 등)을 지어내지 마세요. 대본이 인물의 내면 심리나 독백 위주라면, 무리하게 행동을 만들지 말고 표정과 분위기 위주로 묘사하세요. 등장인물 이미지는 외형 레퍼런스이므로, 각 인물의 얼굴, 체형, 머리색, 헤어스타일, 의상은 레퍼런스와 완전히 동일하게 유지하고, 장면이 바뀌어도 임의로 변경하지 마세요.]";

        var legacyEnInstruction =
          "[Scene / Action / Background / Lighting / Mood: Carefully read the Korean script above. Do NOT describe characters' faces or detailed physical appearance; assume character reference images already exist. Instead, describe in English the characters' actions, movement, gestures, gaze and interactions, the camera framing and distance, the interior/exterior background (space layout and props), the lighting (time of day, direction and color of light), and the overall mood (warm, tense, anxious, serene, etc.) in rich detail so that a director and illustrator can stage the scene. ★STRICT RULE: Strictly adhere to the script. Do NOT invent actions or settings not present in the script. If the script describes internal thoughts or emotions, focus on facial expressions and atmosphere rather than inventing physical actions. Always keep each character's design exactly the same as in the reference images: same face structure, body type, hair color, hairstyle and clothing across all shots, and never change or redesign them. Do not summarize the plot; write it like a shot description.]";
          ...
          chapter.sceneSegments.forEach(function (seg, segIndex) {
            ...
            for (var i = 0; i < cutIndices.length - 1; i++) {
              ...
              (storedCharacters || []).forEach(function (ch) {
                ...
                // 2. Name only match (if 3+ chars, try suffix)
                else if (name.length >= 3) {
                    // Heuristic: If 3 chars (e.g. 김경호), try last 2 chars (경호)
                    // If 4 chars (e.g. 남궁민수), try last 2 chars (민수)? Or last 3?
                    // Let's stick to last 2 chars for 3+ length names for now.
                    var shortName = name.slice(-2); 
                    if (partText.indexOf(shortName) !== -1) {
                        matched = true;
                    }
                }
