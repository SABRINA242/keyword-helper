        ...
        if (editorState.selectedIndex !== -1) {
          ...
          if (applyAllLayersCheckbox && applyAllLayersCheckbox.checked && 
             ...
             // Relative movement for X and Y
                 ...
                 // Value is already updated by the loop, but the input might need refresh?
                 // No, input reflects the current layer's value which we just set correctly via diff
                 // Wait, we should update current layer via loop, so:
                 // The 'val' is absolute value for current layer. 
                 // So we need to calculate diff from OLD value.
                 // But wait, 'currentLayer[prop]' inside the listener IS the old value?
                 // No, we haven't updated it yet. So yes.
             }
             else {
                 editorState.layers.forEach(function(layer) {
                    layer[prop] = val;
