  ...
  function initBangtongTab() {
    ...
    if (resetButton) {
      resetButton.addEventListener("click", function () {
        openConfirmModal(
          "방통이를 초기화하면 현재 관계 축 태그와 썸네일 관련 설정이 모두 지워집니다.\n" +
      ...
      function setResult(textOut) {
        ...
        if (statusEl) {
          if (normalized && normalized.trim()) {
            statusEl.textContent =
              "썸네일 문구 목록이 준비되었습니다. 마음에 드는 문구를 골라 수정해서 사용해 보세요.";
          } else {
            statusEl.textContent =
    ...
    var sunoSequenceInput = document.getElementById("sunoSequenceInput");
    var sunoMusicStyleSelect = document.getElementById("sunoMusicStyleSelect");
    var sunoThemeSelect = document.getElementById("sunoThemeSelect");
    var sunoLoopSelect = document.getElementById("sunoLoopSelect");
    var sunoGenerateStyleButton = document.getElementById("sunoGenerateStyleButton");
    var sunoStyleEnOutput = document.getElementById("sunoStyleEnOutput");
    var sunoTitleOutput = document.getElementById("sunoTitleOutput");
    var sunoStyleEnCopyButton = document.getElementById("sunoStyleEnCopyButton");
    var sunoTitleCopyButton = document.getElementById("sunoTitleCopyButton");
    ...
    var favoriteFonts = [];
    var loadedLocalFonts = [];
    var scannedLocalFonts = [];
    var fontFilterInput = document.getElementById("fontFilterInput");
    var fontFilterText = "";
    var manualFontNameInput = document.getElementById("manualFontNameInput");
    var applyManualFontBtn = document.getElementById("applyManualFontBtn");
      ...
      try {
        ...
        if (data.bgImageData) {
           ...
           img.onload = function() {
             ...
             editorState.bgImageData = data.bgImageData;
             drawCanvas();
           };
    ...
    function renderFontOptions() {
      ...
      // Apply filter text (if any)
      if (fontFilterText && fontFilterText.trim()) {
        var q = fontFilterText.trim().toLowerCase();
        allFonts = allFonts.filter(function (f) {
    ...
    if (fontFilterInput) {
      fontFilterInput.addEventListener("input", function () {
        fontFilterText = fontFilterInput.value || "";
        renderFontOptions();
      });
    }
    ...
    if (applyManualFontBtn && manualFontNameInput) {
      applyManualFontBtn.addEventListener("click", function () {
        if (editorState.selectedIndex === -1) {
          alert("먼저 텍스트 레이어를 선택하세요.");
          return;
        }
        var name = manualFontNameInput.value;
        if (!name) {
          alert("폰트 이름을 입력하세요.");
          return;
        }
        ...
        if (name.indexOf(",") !== -1) {
          ...
          cssValue = "'" + name + "', sans-serif";
        }
        var exists = false;
        for (var i = 0; i < loadedLocalFonts.length; i++) {
          if (loadedLocalFonts[i].family === name) {
            exists = true;
            break;
          }
        }
        if (!exists) {
          for (var j = 0; j < scannedLocalFonts.length; j++) {
            if (scannedLocalFonts[j].family === name) {
              exists = true;
              break;
            }
          }
        ...
        if (!exists) {
          loadedLocalFonts.push({ name: name + " (직접입력)", family: name });
        }
        renderFontOptions();
        fontFamilySelect.value = cssValue;
        editorState.layers[editorState.selectedIndex].fontFamily = cssValue;
        drawCanvas();
        updateFavoriteBtnState();
      });
    }
    ...
    function drawCanvas() {
      ...
      editorState.layers.forEach(function (layer, index) {
        ...
        var lineHeightRatio = parseInt(layer.lineHeight, 10);
        if (isNaN(lineHeightRatio) || lineHeightRatio <= 0) {
          lineHeightRatio = 120;
        }
        ...
        var lineHeightPx = fontSize * (lineHeightRatio / 100);
        ...
        if (strokeWidth > 0) {
          ...
          lines.forEach(function (line, lineIdx) {
            var cursorX = 0;
            var lineY = lineIdx * lineHeightPx;
        ...
        lines.forEach(function (line, lineIdx) {
          ...
          for (var i = 0; i < line.length; i++) {
            ...
            for (var h = 0; h < highlights.length; h++) {
              ...
              if (fillIndex >= range.start && fillIndex < range.end) {
    ...
    function updatePropertiesPanel() {
      ...
      if(lineHeightInput) lineHeightInput.value = layer.lineHeight || 120;
    ...
    function getTextHighlightLimit(text) {
      return (text || "").length;
    }

    function sanitizeHighlights(layer) {
      if (!layer || !Array.isArray(layer.highlights)) return;
      var limit = getTextHighlightLimit(layer.text);
      layer.highlights = layer.highlights
        .map(function (range) {
          var start = Math.max(0, Math.min(limit, parseInt(range.start, 10) || 0));
          var end = Math.max(0, Math.min(limit, parseInt(range.end, 10) || 0));
          return {
            start: start,
            end: end,
            color: range.color || "#ffff00"
          };
        })
    ...
    function applyHighlightToSelection(layer, start, end, color) {
      if (!layer) return;
      if (!layer.highlights) layer.highlights = [];

      // Replace old colors on the same/overlapping selection instead of stacking them.
      layer.highlights = layer.highlights.filter(function (range) {
      ...
      elem.addEventListener("input", function () {
        if (editorState.selectedIndex !== -1) {
          ...
          if (applyAllLayersCheckbox && applyAllLayersCheckbox.checked &&
              prop !== "x" && prop !== "y" && prop !== "color" && prop !== "text") {

            if (prop === "lineHeight") {
              // 1) 모든 레이어에 행간 값 적용
              editorState.layers.forEach(function (layer) {
                layer.lineHeight = val;
              });

              // 2) 레이어들을 Y 기준으로 정렬해서, 동일 간격(val px)으로 다시 배치
              var sortedForGap = editorState.layers.slice().map(function (l) {
                return { l: l };
              });
              sortedForGap.sort(function (a, b) {
                return a.l.y - b.l.y;
              });

              if (sortedForGap.length > 1) {
                var startY = sortedForGap[0].l.y;
                var currentY = startY;
                sortedForGap[0].l.y = startY;

                for (var gi = 1; gi < sortedForGap.length; gi++) {
                  currentY = currentY + val;
                  sortedForGap[gi].l.y = currentY;
                }

                if (editorState.selectedIndex !== -1 && posYInput) {
                  posYInput.value = editorState.layers[editorState.selectedIndex].y;
                }
              }
            } else {
              // 나머지 속성은 값만 전체에 복사
            ...
            var currentVal = parseInt(currentLayer[prop], 10) || 0;
            var diff = val - currentVal;

            if (applyAllLayersCheckbox && applyAllLayersCheckbox.checked) {
              editorState.layers.forEach(function (layer) {
                var base = parseInt(layer[prop], 10) || 0;
                layer[prop] = base + diff;
              });
            } else {
              currentLayer[prop] = val;
            }
          } else {
            editorState.layers[editorState.selectedIndex][prop] = val;
            // If base color changes, maybe clear highlights? 
            // Or keep them? Keeping them is better for "base + highlight" style.
          }
    ...
    bindInput(lineHeightInput, "lineHeight", true);
    ...
    if (layerTextInput) {
      layerTextInput.addEventListener("input", function () {
        if (editorState.selectedIndex !== -1) {
          var currentLayer = editorState.layers[editorState.selectedIndex];
          var prevText = currentLayer.text || "";
          var nextText = layerTextInput.value || "";
          currentLayer.text = nextText;

          // Highlight ranges are stored by character index, so text edits can stale them.
          if (prevText !== nextText) {
            currentLayer.highlights = [];
          }
    ...
    if (highlightTextBtn && layerTextInput) {
      highlightTextBtn.addEventListener("click", function() {
         ...
         applyHighlightToSelection(layer, start, end, color);
    ...
    function buildSunoStyleEn(base, loopKey) {
      var parts = [];
      parts.push(base.style);
      base.instruments.forEach(function (t) {
        parts.push(t);
      });
      parts.push(base.tempo);
      parts.push(base.vocal);
      base.mixing.forEach(function (t) {
        parts.push(t);
      });
      ...
      parts.push("Korean traditional instruments, Korean traditional timbre");
      parts.push("Pure Gugak atmosphere");
      if (loopKey === "40_60") {
        parts.push("short 40–60 second instrumental loop");
        parts.push("seamless loop");
        parts.push("can be cut anywhere without feeling unfinished");
      }
      parts.push("No Intro");
      parts.push("Immediate Start");
      return parts.join(", ");
    }

    function pickGuide(styleKey, themeKey) {
      var styleGuides = {
        fast_exciting: {
          style: "Korean Traditional Folk, Pungmul, Madangnori (Pure Gugak)",
          instruments: [
            "Korean hand gong (Kkwaenggwari, metallic, bright, fast attack)",
            "Korean hourglass drum (Janggu, rhythmic percussion, drum roll, sharp stick sound)",
            "Barrel drum (Buk, resonant, heartbeat, bass drum)",
            "Korean gong (Jing, deep, long decay, brassy)",
            "Korean oboe (Taepyeongso, piercing, high-pitched, loud)"
          ],
          tempo: "Fast & Energetic (Hwimori, Jajinmori), Excited, Bouncy, Rhythmic",
          vocal: "Instrumental Korean percussion groove only, no vocals, no narration",
          mixing: ["Rich, well-blended, energetic percussion", "Warm spatial mixing"]
        },
        sad_sorrowful: {
          style: "Sad Sagueuk Ballad, Cinematic, Emotional Gugak",
          instruments: [
            "Korean fiddle (Haegeum, two-stringed, grainy, sorrowful, violin-like)",
            "Korean bass fiddle (Ajaeng, bowed zither, mellow, deep, cello-like)",
            "Korean bamboo flute (Daegeum, breathy, wind sound)",
            "Korean zither (Gayageum, plucked, clear)"
          ],
          tempo: "Slow, Flowing, Rubato, Melancholic",
          vocal: "Instrumental Haegeum, Ajaeng and Gayageum only, no singing",
          mixing: ["Soft resonance", "Emotional, intimate, grainy texture"]
        },
        faint_hazy: {
          style: "Dreamy Sagueuk, Ambient, Ethereal Gugak",
          instruments: [
            "Daegeum (breathy, air sound)",
            "Sogeum (clear, high-pitched)",
            "Saenghwang (mysterious chords)",
            "Gayageum (plucked, soft)"
          ],
          tempo: "Slow, Floating, Non-rhythmic, Mystical",
          vocal: "Instrumental ambient textures only, no voice",
          mixing: ["Ethereal reverb", "Wide spatial mixing", "Dreamy atmosphere"]
        },
        slow_mystery: {
          style: "Korean Traditional Noir, Suspense, Dark Gugak",
          instruments: [
            "Geomungo (percussive strings, deep resonance, heavy)",
            "Jing (dark resonance, slow decay)",
            "Buk (deep thud)",
            "Ajaeng (low friction, growling bow sound)"
          ],
          tempo: "Slow tension, Heartbeat rhythm, Heavy, Suspenseful",
          vocal: "Instrumental suspense bed, no spoken word",
          mixing: ["Dark ambience", "Deep bass resonance", "Tension"]
        },
        medium_story: {
          style: "Korean Folk Storytelling, Mid-tempo Gugak",
          instruments: [
            "Buk (resonant Korean drum)",
            "Janggu (rhythmic Korean drum)",
            "Daegeum (breathy bamboo flute)",
            "Gayageum (plucked, clear)"
          ],
          tempo: "Medium, Walking pace, Rhythmic (Gutgeori), 90–110 BPM",
          vocal: "Instrumental storytelling groove, no narration",
          mixing: ["Vocal-friendly space for future narration", "Balanced ensemble"]
        },
        grand_epic: {
          style: "Epic Sagueuk Orchestra, War Movie OST (Pure Gugak Version)",
          instruments: [
            "Taepyeongso (piercing, loud Korean oboe)",
            "Massive barrel drum ensemble (Buk, thunderous, epic)",
            "Jing (metallic resonance, long decay)",
            "Piri (strong, nasal, loud bamboo oboe)",
            "Kkwaenggwari (metallic accent, cutting through ensemble)"
          ],
          tempo: "Dynamic, Marching, Grandiose, Powerful",
          vocal: "Instrumental epic percussion and winds only, no choir",
      ...
      var themeGuides = {
        ...
        comedy: {
          ...
          instruments: [
            "Kkwaenggwari (metallic Korean hand gong)",
            "Janggu (rhythmic Korean hourglass drum)",
            "Taepyeongso (piercing Korean oboe)"
          ],
          tempo: "Fast tempo, Bouncy, Rhythmic, Marching",
          vocal: "Instrumental percussive groove only, no narration",
        ...
        destiny: {
          ...
          instruments: [
            "Daegeum (breathy bamboo flute)",
            "Massive Buk ensemble (thunderous Korean barrel drums)",
            "Piri (strong nasal bamboo oboe)"
          ],
          tempo: "Slow build-up to Fast, Dynamic, Marching",
          vocal: "Instrumental only, no heroic choir",
          mixing: ["Wide", "Cinematic", "Majestic"]
        },
        tragedy: {
          style: "Sad Sagueuk Ballad, Cinematic, OST",
          instruments: [
            "Haegeum (grainy, sorrowful Korean fiddle)",
            "Ajaeng (mellow bowed zither)",
            "Gayageum (plucked Korean zither, clear tone)"
          ],
        ...
        revenge: {
          ...
          instruments: [
            "Geomungo (aggressive percussive strings)",
            "Buk (deep Korean barrel drum)",
            "Taepyeongso (piercing Korean oboe)"
          ],
          tempo: "Fast paced, Aggressive, Driving rhythm",
          vocal: "Instrumental only, no spoken-style voice",
          mixing: ["Sharp", "Punchy", "Intense"]
        },
        hope: styleGuides.medium_story,
        despair: {
          style: "Minimalist Sagueuk, Cinematic, Ambient",
          instruments: [
            "Ajaeng (sorrowful bowed zither, deep and grainy)",
            "Daegeum (breathy bamboo flute)",
            "Wind sound texture"
          ],
          tempo: "Very slow, Dragging, Stillness",
          vocal: "Instrumental only, no spoken word, no humming",
          mixing: ["Empty space", "Cold and desolate atmosphere"]
        }
      };

      if (styleKey && styleKey !== "auto" && styleGuides[styleKey]) {
        return styleGuides[styleKey];
      }
      if (themeKey && themeKey !== "auto" && themeGuides[themeKey]) {
        return themeGuides[themeKey];
      }
      return styleGuides.medium_story;
    }

    function buildSunoTitle(seq, themeKey) {
      var text = seq || "";
      ...
      var suffix = suffixMap[themeKey] || "사극 인트로";
      if (!text) return suffix;
      return text + " · " + suffix;
    }

    function copyTextSimple(value, emptyMessage) {
      var text = value || "";
      if (!text.trim()) {
        if (emptyMessage) {
          alert(emptyMessage);
    ...
    if (sunoGenerateStyleButton && sunoStyleEnOutput) {
      sunoGenerateStyleButton.addEventListener("click", function () {
        var seq = sunoSequenceInput ? sunoSequenceInput.value.trim() : "";
        var styleKey = sunoMusicStyleSelect ? sunoMusicStyleSelect.value : "auto";
        var themeKey = sunoThemeSelect ? sunoThemeSelect.value : "auto";
        var loopKey = sunoLoopSelect ? sunoLoopSelect.value : "auto";
        var guide = pickGuide(styleKey, themeKey);
        var styleEn = buildSunoStyleEn(guide, loopKey);
        sunoStyleEnOutput.value = styleEn;
        sunoStyleEnOutput.focus();
        sunoStyleEnOutput.select();
        if (sunoTitleOutput) {
          sunoTitleOutput.value = buildSunoTitle(seq, themeKey);
        }
      });
    }

    if (sunoStyleEnCopyButton && sunoStyleEnOutput) {
      sunoStyleEnCopyButton.addEventListener("click", function () {
        copyTextSimple(
          sunoStyleEnOutput.value,
          "먼저 스타일 프롬프트를 생성해 주세요."
        );
      });
    }

    if (sunoTitleCopyButton && sunoTitleOutput) {
      sunoTitleCopyButton.addEventListener("click", function () {
        copyTextSimple(sunoTitleOutput.value, "먼저 제목을 생성해 주세요.");
      });
    }
