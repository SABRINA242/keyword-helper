    ...
    storedChapters.forEach(function (chapter, chapterIndex) {
      ...
      segments.forEach(function (segment, segmentIndex) {
        ...
        function updatePerScene() {
          ...
          chapter.sceneConfig[key] = scenes;
          saveStoredChapters();
        }
  ...
  function initShintongTab() {
    ...
    function rebuildSceneCharacterControls() {
      ...
      indexes.forEach(function (startPos, sceneIndex) {
        ...
        var imagePre = document.createElement("pre");
        imagePre.className = "scene-block-text-image";
        imagePre.textContent = imageText;
  ...
  function initScriptSettingsTab() {
    ...
    if (buildScenePromptButton) {
      buildScenePromptButton.addEventListener("click", function () {
        ...
        storedChapters.forEach(function (chapter, chapterIndex) {
          // Only process the last chapter to avoid re-analyzing previous ones
          if (chapterIndex !== storedChapters.length - 1) return;
          ...
          chapter.sceneSegments.forEach(function (seg, segIndex) {
            ...
            var key = label;
            var scenesCount = sceneConfig[key] && sceneConfig[key] > 0 ? sceneConfig[key] : 1;
            ...
            var cutPositions = [0];
            for (var ci = 1; ci < scenesCount; ci++) {
              var target = Math.floor((segLen * ci) / scenesCount);
              var prev = cutPositions[cutPositions.length - 1];
              if (target <= prev + 1) {
                target = prev + 1;
              }
              if (target >= segLen) {
              ...
              if (boundary > prev + 1 && boundary < segLen) {
                cutPositions.push(boundary);
              }
            }
            if (cutPositions[cutPositions.length - 1] !== segLen) {
              cutPositions.push(segLen);
            }

            for (var i = 0; i < cutPositions.length - 1; i++) {
              var partStart = cutPositions[i];
              var partEnd = cutPositions[i + 1];
              if (partEnd <= partStart) continue;
              var partText = segText.slice(partStart, partEnd).trim();
              ...
              (storedCharacters || []).forEach(function (ch) {
                ...
                if (partText.indexOf(name) !== -1) {
                  if (partCharacters.indexOf(name) === -1) {
              ...
              parts.push({
                ...
                partText: partText,
                partCharacters: partCharacters.slice(),
